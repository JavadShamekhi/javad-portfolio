...
export default function TimelineItem({ title, org, period, description, bullets }: TimelineItemProps) {
  return (
    ...
    >
      <span className="absolute left-0 top-2 bg-blue-600 dark:bg-blue-500 w-3 h-3 rounded-full"></span>
      <div className="flex flex-wrap items-baseline gap-x-2">
        <h4 className="font-semibold text-lg">{title}</h4>
        {org ? <span className="text-gray-600 dark:text-gray-300">• {org}</span> : null}
        {period ? <span className="ml-auto text-sm text-gray-500 dark:text-gray-400">{period}</span> : null}
      </div>
